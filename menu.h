...
{
  sprites.drawSelfMasked(28, 8, IMG_TITLE, 0);
}
...
{
  drawTitleScreen();
  print_progmem(29, 52, text_input);
  print_progmem(59 + (wii_on * 23), 52, text_pointer);
  if (arduboy.justPressed(RIGHT_BUTTON)) {wii_on = 1;}
  if (arduboy.justPressed(LEFT_BUTTON)) {wii_on = 0;}
