...
#define TYPE_XBOT   5
#define TYPE_PUMPKIN 6

#define ENEMY_XBOT_LT 1
#define ENEMY_XBOT_RT 2

#define PUMPKIN_ANIMATE 1
...
//*******************
LevelElement pumpkin_handle(LevelElement element)
{
  ...
  if (element.state > STATE_HIDDEN)
  {
    ...
    sprites.drawOverwrite(element.x - backdrop.view_x, element.y - backdrop.view_y, IMG_PUMPKIN, element.step);
  }
  return element;
}
...
void level_element_handle()
{
  ...
  {     
    switch (levelElements[i].type) {
      ...
      case TYPE_PUMPKIN:
      levelElements[i] = pumpkin_handle(levelElements[i]);
      break;
    }
